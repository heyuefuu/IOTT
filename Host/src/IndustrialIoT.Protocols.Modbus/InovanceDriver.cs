...
[ProtocolDriver(ProtocolType.Inovance, "Inovance", "汇川")]
public sealed class InovanceDriver : IProtocolDriver, IAddressSpaceBrowser
{
    private const ushort DefaultStringLength = 16;
    private readonly ILogger<InovanceDriver> _logger;
    private readonly SemaphoreSlim _semaphore = new(1, 1);
    private IInovanceClient? _client;
    private ConnectionState _state = ConnectionState.Disconnected;
    private InovanceSeries? _series;

    public InovanceDriver(ILogger<InovanceDriver> logger) => _logger = logger;
    public ProtocolType Protocol => ProtocolType.Inovance;
    ...
    public async Task<ConnectionResult> ConnectAsync(DeviceConnectionConfig config, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            var station = byte.TryParse(Get(config, "Station"), out var value) ? value : (byte)1;
            var client = new InovanceTcpNet(_series.Value, config.Host, config.Port, station);
            client.ConnectTimeOut = (int)config.ConnectTimeout.TotalMilliseconds;
            client.ReceiveTimeOut = (int)config.ReadTimeout.TotalMilliseconds;
            ApplyExtendedProperties(client, config, _series);
            var result = await client.ConnectServerAsync();
            if (!result.IsSuccess) throw new InvalidOperationException(result.Message);
            _client?.Dispose();
            _client = new HslInovanceClientAdapter(client);
        ...
        {
            CleanupClient();
            _logger.LogError(ex, "Inovance connection failed");
    ...
    public async Task<bool> PingAsync(CancellationToken ct = default)
    {
        ...
        try { return (await GetReadWriteNet().ReadUInt16Async(InovanceAddressSpace.GetPingAddress(_series.Value))).IsSuccess; }
        catch { return false; }
    ...
    public async Task<TagValue> ReadTagAsync(string address, DataType dataType, CancellationToken ct = default)
    {
        ...
        catch (Exception ex) when (ex is not OperationCanceledException) { _logger.LogError(ex, "Inovance read failed at {Address}", address); return BadTag(address, dataType, ex.Message); }
        finally { _semaphore.Release(); }
    }
    ...
    public async Task<WriteResult> WriteTagAsync(string address, DataType dataType, object value, CancellationToken ct = default)
    {
        ...
        catch (Exception ex) when (ex is not OperationCanceledException) { _logger.LogError(ex, "Inovance write failed at {Address}", address); return new() { Success = false, ErrorMessage = ex.Message }; }
        finally { _semaphore.Release(); }
    }
    ...
    private static void ApplyExtendedProperties(InovanceTcpNet client, DeviceConnectionConfig config, InovanceSeries? series)
    {
        if (series.HasValue) client.Series = series.Value;
        if (byte.TryParse(Get(config, "Station"), out var station)) client.Station = station;
        if (bool.TryParse(Get(config, "AddressStartWithZero"), out var zero)) client.AddressStartWithZero = zero;
        if (bool.TryParse(Get(config, "IsStringReverse"), out var reverse)) client.IsStringReverse = reverse;
        if (Enum.TryParse<DataFormat>(Get(config, "DataFormat"), true, out var format)) client.ByteTransform.DataFormat = format;
    }

    private InovanceSeries RequireSeries() => _series ?? throw new InvalidOperationException("Inovance Series is required.");
    private IInovanceClient GetReadWriteNet() => _client ?? throw new InvalidOperationException("Inovance driver is not connected");
    private void EnsureConnected() { if (_state != ConnectionState.Connected || _client is null) throw new InvalidOperationException("Inovance driver is not connected"); }
